...
class QuizUI:
    def __init__(self, master):
        ...
        # self.correct_answers = ["C. Paris", "B. JavaScript", "B. Central Processing Unit"]
        self.correct_answers =answers = ["C. Paris", "B. Mars", "C. William Shakespeare", "D. Pacific", "C. Carbon Dioxide", "C. 7", "C. 2", "B. Au", "B. China", "D. Tennis", "C. Leonardo da Vinci", "C. Kidney", "C. Mandarin", "B. Piano", "B. Diamond"]
    ...
    def show_result(self):
        ...
        score_msg = f"You scored {correct} out of {self.total_qs}!"
        self.clear_screen()
        tk.Label(self.master, text="Quiz Completed", font=("Helvetica", 20, "bold"),
                 bg=self.themes[self.theme]["bg"], fg=self.themes[self.theme]["fg"]).pack(pady=20)
        tk.Label(self.master, text=score_msg, font=("Helvetica", 16),
                 bg=self.themes[self.theme]["bg"], fg=self.themes[self.theme]["fg"]).pack(pady=10)
        tk.Button(self.master, text="Exit", command=self.master.quit,
                  font=("Helvetica", 12), bg="#c62828", fg="white").pack(pady=20)
